...
export default function PiToolCard({
  ...
  useEffect(() => {
    if (Array.isArray(status?.availableModels)) setModels(status.availableModels);
    if (status?.pi?.defaultModel) setDefaultModel(status.pi.defaultModel);
    if (status?.pi?.thinkingLevel) setThinkingLevel(status.pi.thinkingLevel);
  }, [status]);

  useEffect(() => {
    if (!defaultModel && models.length > 0) setDefaultModel(models[0].id);
  }, [models, defaultModel]);
  ...
  const statusLabel = useMemo(() => {
    ...
    return matchKnownEndpoint(configuredUrl, {
      tunnelPublicUrl,
      tailscaleUrl,
      cloudUrl: cloudEnabled ? CLOUD_URL : "",
    }) ? "configured" : "other";
  }, [status, configuredUrl, tunnelPublicUrl, tailscaleUrl, cloudEnabled]);
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/cli-tools/pi-settings", {
        ...
        body: JSON.stringify({ baseUrl: effectiveBaseUrl, apiKey: keyToUse, defaultModel, thinkingLevel }),
      });
      const data = await res.json();
        ...
        <div className="mt-4 pt-4 border-t border-border flex flex-col gap-4">
          ...
          {!checking && status && !status.installed && (
            <div className="flex flex-col gap-3 p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
              ...
              <div className="pl-9"><Button variant="outline" size="sm" onClick={() => setShowInstallGuide((value) => !value)}>{showInstallGuide ? "Hide" : "How to Install"}</Button></div>
              {showInstallGuide && <code className="block px-3 py-2 bg-black/5 dark:bg-white/5 rounded font-mono text-xs break-all">npm install -g @earendil-works/pi-coding-agent</code>}
            </div>
          )}
          ...
          {!checking && status?.installed && (
            <>
              <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr] sm:items-center sm:gap-2">
                <span className="text-xs font-semibold text-text-main sm:text-right sm:text-sm">Endpoint</span><span className="material-symbols-outlined hidden text-text-muted text-[14px] sm:inline">arrow_forward</span>
                <BaseUrlSelect value={endpoint || ensureV1(baseUrl)} onChange={setEndpoint} requiresExternalUrl={tool.requiresExternalUrl} tunnelEnabled={tunnelEnabled} tunnelPublicUrl={tunnelPublicUrl} tailscaleEnabled={tailscaleEnabled} tailscaleUrl={tailscaleUrl} cloudEnabled={cloudEnabled} cloudUrl={CLOUD_URL || ""} currentUrl={configuredUrl} />
              </div>
              <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr] sm:items-center sm:gap-2">
                <span className="text-xs font-semibold text-text-main sm:text-right sm:text-sm">API Key</span><span className="material-symbols-outlined hidden text-text-muted text-[14px] sm:inline">arrow_forward</span>
                <ApiKeySelect value={selectedApiKey} onChange={setSelectedApiKey} apiKeys={apiKeys} cloudEnabled={cloudEnabled} />
              </div>
              <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr] sm:items-center sm:gap-2">
                <span className="text-xs font-semibold text-text-main sm:text-right sm:text-sm">Default model</span><span className="material-symbols-outlined hidden text-text-muted text-[14px] sm:inline">arrow_forward</span>
                <select value={defaultModel} onChange={(event) => setDefaultModel(event.target.value)} className="w-full min-w-0 px-2 py-2 bg-surface rounded text-xs border border-border focus:outline-none focus:ring-1 focus:ring-primary/50 sm:py-1.5">
                  {models.map((model) => <option key={model.id} value={model.id}>{model.name || model.id} — {model.id}</option>)}
                </select>
              </div>
              <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr] sm:items-center sm:gap-2">
                <span className="text-xs font-semibold text-text-main sm:text-right sm:text-sm">Thinking</span><span className="material-symbols-outlined hidden text-text-muted text-[14px] sm:inline">arrow_forward</span>
              ...
              {status?.pi?.ownsDefaults === false && <div className="rounded bg-yellow-500/10 px-3 py-2 text-xs text-yellow-700 dark:text-yellow-300">Pi already has another default provider. 9Router will be added without replacing that default.</div>}
              {status?.pi?.models?.length > 0 && <div className="rounded bg-surface/40 px-3 py-2 text-xs text-text-muted">{status.pi.models.length} routed model{status.pi.models.length === 1 ? "" : "s"} in {status.configPath}{status.synced ? " · catalog refreshed" : ""}</div>}
              ...
              <div className="flex flex-wrap gap-2">
                <Button size="sm" onClick={handleApply} disabled={applying || !effectiveBaseUrl || models.length === 0}>{applying ? "Applying..." : "Apply 9Router"}</Button>
